import sys
import qdarkstyle
from PySide2.QtWidgets import (QApplication, QMainWindow, QLabel, QLineEdit, QVBoxLayout, QHBoxLayout, QPushButton, QWidget, QTextEdit, QComboBox)
...
class MainWindow(QMainWindow):
    ...
    def GetAnswer(self):
        ...
        if QueryText != "":
            self.TextEdit.setText(howdoi.howdoi(QueryText  + self.engine))

    def JsonAnswer(self):
        QueryText = self.input.text()
        if QueryText != "":
            self.TextEdit.setText(howdoi.howdoi(QueryText + self.engine + " -j"))
